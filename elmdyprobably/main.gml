...
with (instance_create(0, 0, obj_custom_object_ext))
{
	...
	event.step[0] = @'
		...
		with snickexe
		{
			...
			{
				if rectangle_in_rectangle(obj_parryhitbox.bbox_left, obj_parryhitbox.bbox_top, obj_parryhitbox.bbox_right, obj_parryhitbox.bbox_bottom, bbox_left, bbox_top, bbox_right, bbox_bottom)
				{
	...
	event.draw_gui[0] = @'
		...
		{
			with postivenumbers[i]
			{
				alpha -= 0.03;
				if alpha <= 0
					array_delete(other.postivenumbers, i, 1);
				y -= 0.5;
				draw_set_halign(fa_left);
				draw_set_alpha(alpha);
				draw_set_color(#58C000);
	...
	event.room_start[0] = @'
		...
		if global.laps >= 3 && global.lap4times[$ room_get_name(room)] != undefined && global.visitedrooms[$ room_get_name(room)] == undefined
		{
			...
			{
				x : 124,
				y : display_get_gui_height() - 96 - offset,
				alpha : 1.5,
